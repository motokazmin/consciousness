...
**Типы активности (slug в `sessions.tag`):** системные — `meditation`, `relaxation`, `test`, `focus`, `scroll`, `untagged`; плюс пользовательские записи в `session_types` (`is_custom=1`).

- **Seed:** при пустой таблице `session_types` заполняется из [`hrv_core/session_types.py`](hrv_core/session_types.py).
- **Runtime (веб):** списки в форме и фильтрах — из таблицы `session_types` через `GET /api/session-types`; новый тип — «Новая активность…» → `POST /api/session-types` (slug + label).
- **Legacy:** старый slug `rest` заменён на `relaxation`; при миграции данных — `UPDATE sessions SET tag='relaxation' WHERE tag='rest'`.
...
| `/api/sessions` | POST/GET | Старт / список сессий (фильтры: participant, tag, даты) |
...
```
consciousness/
├── hrv_core/           # Ядро: источники, pipeline, БД, session_types (seed)
